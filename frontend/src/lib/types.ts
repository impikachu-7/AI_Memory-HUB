/** AI Memory Hub interface types — explicit ownership and user-scoped boundaries. */

export type ProviderId = "google" | "openai" | "anthropic" | "ollama" | "deepseek" | "groq" | "openrouter";
export type MemoryStatus = "active" | "archived";

export interface ProviderSummary { id: ProviderId; name: string; connected: boolean; local?: boolean; modelCount: number; }
...
export interface AuthUser { id: string; email: string; full_name: string | null; is_email_verified: boolean; }
export interface TokenResponse { access_token: string; token_type: "bearer"; user: AuthUser | null; }
export interface ProfileUpdate { full_name: string | null; }
export interface ApiErrorPayload { message: string; code?: string; status?: number; }
